...
다음 세팅 가이드의 본문을 system 프롬프트의 형식으로 작성하세요.

## 세팅 메타

- 슬러그: {{scenario.slug}}
- 제목 후보: {{scenario.title_ko}}
- 라이프스타일: {{scenario.persona.title_ko}} ({{scenario.persona.slug}})
...
- ✅ "이 제품은 ~한 특징", "예산 30만원에서 추천", "자취생에게 적합"
...
**분량 (세션 #48 — 지금까지 요청치의 1.5~2배를 써 왔다. 아래 예산을 지켜라):**
- 본문 총 **2,600~3,200자** (한국어, 마크다운 기호 제외). 상한을 넘기지 마라.
- 섹션별 예산: §1·§2 각 250~350자 · **§3 상품 각 120~180자**(8개면 1,000~1,400자) ·
  §4 표 · §5 150~250자 · §6 FAQ 4~5개 각 80~120자.
- ★길게 쓴다고 좋은 글이 아니다. 같은 말을 바꿔 반복하거나 일반론을 늘리지 마라 —
  그건 오히려 non-commodity 의무 위반이다. 정보 밀도를 높이고 분량은 예산 안에서 끝내라.

8섹션 모두 포함. 1인칭 금지 + non-commodity 의무.
**disclosure·광고 공시 문구를 본문에 절대 쓰지 마라** — "[광고 공시]", "본 페이지는 쿠팡 파트너스…"
같은 문구는 시스템이 표준 문구로 자동 삽입한다. 본문은 "# 페이지 제목"부터 시작.
META-JSON `featured_products`에는 본문에서 실제 추천·언급한 상품의 고유 ID(deeplink_slug)를 나열.
**가급적 8개**(저가대 4 + 고가대 4, 예산 골고루)를 본문 §3·§4에서 추천·언급하고 그 ID를 모두 나열.
수집 상품이 8개 미만이면 있는 만큼. 본문 가격은 반드시 위 목록 가격과 일치(안 적은 상품은 제외).
...
- `picks`: `featured_products`의 **각 상품(최대 8개)**마다 `{slug(고유 ID 그대로), pros(2개), cons(1개), for(잘 맞는 사람)}`.

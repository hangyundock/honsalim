...
def build_seo_directive(
    ...
    *,
    ...
    lines = [
        ...
        # ★#48 라이브: 섹션 제목이 system 프롬프트의 8섹션 표준("## 1. 누구를 위한 가이드인가")이라
        #   모델이 그 문구를 그대로 복사하고 키워드를 안 넣는 일이 반복됐다(최근 3회 중 2회 0개).
        #   추상적 "소제목에 넣어라" 대신 **어느 소제목을 어떻게 고칠지**를 그대로 보여준다.
        f'  가장 쉬운 방법: 1번 섹션 제목을 "## 1. 누구를 위한 {primary} 가이드인가"로 쓰면 된다.',
        "  다만 3개 이상은 도배이니 1~2개까지만.",
        # ★#48 B2 — 키워드 글은 절대 횟수 하한(길이 무관), 카테고리 페이지는 기존 % 밴드.
        (
            f'- (필수) 본문 전체에서 "{primary}"를 **총 {min_count}회 이상** 사용 — '
            f"**{min_count}회 미만이면 '미달'로 탈락**한다. 서로 다른 문단·소제목·FAQ에 나눠 "
            f"자연스럽게 쓰면 충분하다. 상한: **1,000자당 {high}회를 넘기면 '도배'로 탈락**한다."
            if min_count
            else f"- (필수) 분량: 본문 1,000자당 **최소 {low}회 이상 ~ 최대 {high}회 이하**. "

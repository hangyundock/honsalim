...
DEFAULTS: dict[str, Any] = {
    ...
    # LLM 단가(백만 토큰당 USD) — ★기본 0 = 미설정. 주인이 실제 단가를 넣기 전까지 비용은
    # 0으로 집계되고 **토큰 수만** 기록된다(모르는 단가를 지어내지 않음·§0 가짜 지표 금지).
    "llm_price_in_per_1m": 0.0,
    "llm_price_out_per_1m": 0.0,
    "seo_max_attempts": 2,  # SEO 게이트 재생성 상한 (카테고리 페이지 — 비용 방지 CLAUDE §6)
    # 키워드 글 5게이트 재생성 상한 (세션 #33 무인 자가복원 — 비용 방지).
    # ★세션 #48 2→3(주인 승인): 상한이 2면 밀도 외 게이트(1인칭 등)가 한 번만 걸려도 남는 기회가
    # 1회뿐이라 밀도 교정을 못 하고 반려된다 — 07-31 라이브 3회 중 2회가 이 경로로 실패했다.
    ...
    # ── 키워드 공략 구간(세션 #51 근본수정 — '이길 수 있는 키워드'로 하강) ──────────────
    # ★문제: 하한 2000이 네이버 후보의 94.8%를 잘라내, 시스템이 **가장 경쟁이 심한 상위 5%**
    #   에서만 키워드를 골랐다(라이브 실측 1,980개 중 2000미만 1,878개 폐기). 결과 = 발행 22편
    #   전부 "○○추천" head 키워드, GSC 평균순위 18.2위(2페이지)·3개월 클릭 2.
    # ★1차안 기각(실데이터 반증): "검색량 상한(cap)을 낮춰 **경쟁도**를 결정자로" 안은 폐기했다.
    #   네이버 경쟁도는 '광고' 경쟁도라 분포가 쏠려 변별력이 없다 — 300~2000 구간 305개 중
    #   '낮음'이 **단 1개**(중간 117·높음 187). cap=1000으로 돌리니 상위 15개가 전부 동점(600)이
    #   되어 순서가 사실상 무작위가 됐다. 그래서 **검색량 자체를 난이도 지표로** 쓴다(검색량이
    #   클수록 강자가 점유 — 실제로 우리가 진 22편이 전부 고검색량 구간이었다).
    # ★공략 구간 = [floor, ceiling]. 이 안에서는 기존 winnable(검색량 곱하기 경쟁가중)으로 정렬하므로
    #   구간 내 상위 검색량이 먼저 나간다. 사이트 권위가 쌓이면(1페이지 진입 확인 후) ceiling을
    #   올려 head로 확장한다 — 그래서 상수가 아니라 설정이다.
    # ★변경 효과는 `keyword-recommend --dry-run`으로 비용 0 확인(라이브 발행 전 미리보기).
    "keyword_volume_floor": 300,  # 하한 — 0~300은 트래픽 기대 0(실측 후보의 55%가 0~100)
    "keyword_volume_ceiling": 2000,  # 상한 — 0이면 상한 없음(옛 동작). 신규 도메인 보호선
    # ── 채널 ──

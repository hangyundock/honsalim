...
REDIRECTS: list[tuple[str, str]] = [
    # (제거 #44) 게이밍의자 kw-e3d08a2c → 의자 카테고리 301(#31 타입 흡수)은 글 재발행(주인
    # 결정)으로 폐기 — 키워드 글이 카테고리와 공존하는 현행 모델(#39~). slug가 sha1(키워드)로
    # 결정적이라 리다이렉트가 남으면 재발행 글이 좀비(URL 가로채기)가 된다.
    # 노트북받침대 글 → 모니터받침대 카테고리(같은 36개 상품·중복이라 카테고리로 흡수, #35 — 유지)

...
#
# ★세션 #48 — 'N년 사용' 패턴의 **오탐 제거**(기준 완화 아님, 주인 승인).
#   라이브 07-31 실측: "스테인리스·철제 프레임은 **2~3년 사용**에도 견고하다는 후기가 많지만…"이
#   1인칭으로 반려됐다. 이는 운영자의 사용 경험 주장이 아니라 **후기를 인용한 객관 서술**인데,
#   내구성을 설명하는 자리에서 반복 발생해 재시도 기회를 계속 소모했다(3회 중 2회).
#   게이트의 목적은 "직접 써봤다"는 거짓 경험 주장을 막는 것이므로, 'N년 사용' 뒤에 **부사격
#   조사·명사**가 붙어 경험 주장이 될 수 없는 형태만 좁게 예외 처리한다.
#     막힘(그대로 fail): "6개월 사용해보았습니다" · "3년 사용했습니다" · "2년 썼"
#     통과(오탐 해소):   "2~3년 사용에도 견고" · "3년 사용 시 마모" · "2년 사용 기간"
#   '내/우리/써본/사용해보' 등 명시적 1인칭 표지는 아래 다른 패턴이 그대로 전부 잡는다.
_USAGE_NON_EXPERIENTIAL = r"에|시|기간|량|감|환경|조건|빈도"
FIRST_PERSON_PATTERNS = (
    ...
    rf"(\d+개월|\d+년)\s*(?:썼|사용(?!\s*(?:{_USAGE_NON_EXPERIENTIAL})))",
)

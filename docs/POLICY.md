...
#### 3-1-3. 1인칭·직접 경험 (DECISIONS L1·L3 [확정 세션 #6 재설계])

> **위키바이형 정보 분석 기본 톤**. 1인칭은 본인 실보유 5~10개 제품에만 선택적 액센트.
> 본 검사 폐기: ~~"직접 사진 없이 1인칭 사용 → fail (E7·E8 위반)"~~ → L3 기본 차단으로 강화.

| 검사 | 통과 조건 |
|------|----------|
| 1인칭 표현 ("써본 결과"·"내가 사용"·"우리집에서") | payload `owned_products: [<sku>, ...]` 메타에 해당 상품 명시 + 그 상품 영역에서만 사용 |
| `owned_products` 메타 없이 1인칭 사용 | **fail** (L3 위키바이형 톤 위반) |
| `owned_products` 명시 + 페르소나 사진 1+ (사이트 전체 6~9장 풀에서 재사용) | pass |
| 특정 상품에 대한 강한 칭찬 ("최고"·"무조건") | `owned_products` 명시 + 사용 기간 명시 필수 |
...
#### 3-1-7. 한국어 1인칭 허용 범위 (E8 폐기·L1·L3 [확정 세션 #6])

> ~~E8 한국어 1인칭 전면 허용~~ → **L1·L3 선택적 액센트로 축소**. 본 절은 액센트 사용 시 가이드.

위키바이형 정보 분석이 기본. 1인칭 사용 시:

- payload `owned_products` 메타에 해당 상품 SKU 명시 의무
- 본인 실보유 5~10개 제품에만 한정 (전 시나리오에서 동일 제품 반복 등장 자연스러움)
- 사용 기간 ("3개월 사용"·"지난 겨울") 명시 권장
- 과장 ("내 인생 최고") 회피
- 본인 외 가족 사용 ("우리집 아이가") 신중 (의료·연령 민감)
- 페르소나 인테리어 사진 6~9장 풀에서 재사용 (시나리오마다 신규 촬영 X)
...
### 3-3. 사용자 직접 사진 메타 (DB §9, L2 [확정 세션 #6 재설계])

> ~~D5 시나리오당 직접 사진 1~3장 의무~~ → **L2 페르소나 사진 6~9장 사이트 전체 의무**.
> 시나리오마다 페르소나 사진 1~2장 재사용 (신규 촬영 X).

페르소나 인테리어 사진 검증 (`images` 테이블 row):
- `source_type = 'user_photo'`
- `persona_slug` 명시 (jachisin·jaetaek·jeongchakja 중 1)
- `license_note` 비어있지 않음
- `alt_text_ko` 비어있지 않음
- `width_px > 0` AND `height_px > 0`

각 페르소나당 2~3장 사전 등록 의무 (사이트 전체 6~9장). 시나리오 본문에서 페르소나 사진 1+ 참조 시 truth 게이트 가산점.

상품 사진은 **쿠팡 공식 위젯** 자동 처리 (저작권 회색지대 회피 — 쿠팡 CDN 직접 다운로드 X).

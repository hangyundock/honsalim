...
### 2-2. 첫머리 disclosure 표준 문구 — 제휴처 인지형 [확정 2026-05-30]

**한국어 단일** 한 줄 안내, 글 최상단. **글이 실제 추천한 상품의 제휴처를 정확히 공시**
(공정위 정확성 — 알리 상품만 추천한 글에 쿠팡만 명시하면 부정확). 문구 패턴 동일, 제휴처명만 교체:
...
[쿠팡·알리 혼합 글]
이 글에는 쿠팡 파트너스 및 AliExpress 어필리에이트 활동의 일환으로 일정 수수료를 제공받습니다.
(구매자에게 추가 비용은 발생하지 않습니다.)
```

구현: `writer.article_writer.apply_disclosure(body, sources)` — featured 상품 제휴처로 자동 선택.
검증: `validator.disclosure` 첫머리 = '수수료' + 제휴처명(쿠팡 파트너스 또는 AliExpress) 최소 1개.
...
#### 3-1-3. 1인칭 완전 차단 (DECISIONS L3·L5 [확정 세션 #6 2차 재변경])

> 사용자 직접 사진 일체 없음 + AI 생성 이미지 (L2). AI 이미지로 "내가 써봤다" 1인칭 게재 = 거짓 광고·공정위 위반.
> ~~L3 초안 owned_products 메타 허용~~ 폐기 → **1인칭 완전 차단**.

| 검사 | 통과 조건 |
|------|----------|
| 1인칭 표현 ("써본 결과"·"내가 사용"·"우리집에서"·"3개월 사용") | **무조건 fail** (위키바이형 정보 분석 톤 강제) |
| 본문 모든 1인칭 패턴 | 작성 단계에서 회피 의무 — Claude API prompt에 1인칭 금지 명시 |
...
#### 3-1-7. 한국어 1인칭 전면 폐기 (E8·L5 [확정 세션 #6 2차])

> ~~E8 한국어 1인칭 허용~~ → **전면 폐기 (L5)**. 사용자 사진 없음 + AI 생성 이미지 결정 → 1인칭 = 거짓 광고.

본문 모든 1인칭 표현 차단. 위키바이형 정보 분석·3인칭 톤만 허용:
- "이 제품은 ~한 특징", "예산별 비교 ~", "페르소나 ~에게 적합 ~"
- "본인", "내가", "내", "우리", "저희", "우리집" 등 1인칭 일체 차단
- 사용 기간 표현 ("3개월 사용"·"지난 겨울") 차단
- Claude API enricher prompt에 1인칭 회피 명시 의무
...
### 3-3. AI 생성 이미지 메타 (DB §9, L2·L6·L7·L8 [확정 세션 #6 2차])

> ~~D5 직접 사진 의무·L2 초안 페르소나 사진 6~9장~~ 전면 폐기. 사용자 직접 촬영 일체 없음.
> AI 생성 이미지 (Google Imagen 4 Fast) + 쿠팡 공식 위젯으로 전면 대체.
> 도구·API 명세: `docs/IMAGE_GENERATION.md` [확정].

페르소나·시나리오 이미지 검증 (`images` 테이블 row):
- `source_type = 'ai_generated'`
- `ai_model` = 'imagen-4.0-fast-generate-001' 또는 'gemini-2.5-flash-image'
- `prompt_used` 비어있지 않음 (재현·감사 가능)
- `persona_slug` 명시 (jachisin·jaetaek·jeongchakja 중 1, hero/intro 용)
- `alt_text_ko` 비어있지 않음 (AI 생성 일러스트레이션임을 시각장애인 접근 시 명시)
- `width_px > 0` AND `height_px > 0`

추가 의무 (L7 [확정]):
- 글 footer에 "이미지는 AI 생성 일러스트레이션" 한 줄 명시
- 인물 이미지 자제 (인테리어·분위기만)

상품 이미지 (L8 [확정]):
- **쿠팡 공식 위젯 embed**만 사용
- Imagen으로 상품 이미지 생성 금지 (실제 제품 정확성·법규 — 가짜 상품 이미지 = 소비자 기만)
- 쿠팡 CDN 직접 다운로드 금지 유지
...
**운영자명·이메일 [확정 DECISIONS M2-2·M2-6, 세션 #7]**:
- 운영자명 (필명) = **혼살다** (실명 비공개, 사생활 보호 + E-E-A-T author 부분 충족)
- 이메일 = **dugihappyending@gmail.com**
- footer + about 페이지 일관 표기 의무

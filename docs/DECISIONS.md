...
- **L2. 사진 정책 — 페르소나 인테리어 분위기만 [확정]**: 사용자 직접 촬영 사진은 **페르소나 3개의 인테리어·분위기 사진 2~3장 = 사이트 전체 6~9장**으로 한정. 시나리오마다 페르소나 사진 1~2장 재사용. 상품 사진은 **쿠팡 공식 위젯·이미지**로 일괄 처리 (쿠팡 CDN 직접 다운로드 X — 위젯 embed만). D5의 "1편당 1~3장 의무"는 폐기 — 시나리오당 사진 의무 없음 (페르소나 사진 재사용 OK) — 세션 #6
- **L3. validator/truth 1인칭 기본 차단 [확정]**: 본문에서 1인칭 표현 (POLICY §3-1-3 패턴) 감지 시 **기본 fail** (위키바이형 톤 강제). 단, payload에 `owned_products: [<sku>, ...]` 메타 명시 + 해당 상품 영역에서만 1인칭 사용 시 통과. 본인 실보유 제품 식별·1인칭 액센트 위치 제한 — 세션 #6
...
- **L5. E8 축소·D5 완화 [확정]**: E8 (한국어 1인칭 허용)은 **선택적 액센트로 축소** (전면 허용 X). D5 (직접 사진 1~3장 의무)는 **시나리오당 의무 폐기**, 페르소나 사진 6~9장 사이트 전체 의무로 대체. POLICY §3-1-3/§3-1-7/§3-3 + DESIGN §11-2 정합 갱신 의무 — 세션 #6
...
| 세션 #6 (2026-05-28) | E8 한국어 1인칭 전면 허용 | 수백~수천 제품 직접 보유 불가능 (사용자 지적) → L1·L3 선택적 액센트로 축소 |
| 세션 #6 (2026-05-28) | D5 시나리오당 직접 사진 1~3장 의무 | 동일 현실 정합 문제 → L2 페르소나 사진 6~9장 사이트 전체 의무로 대체 |

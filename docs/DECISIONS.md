...
## Q. 도메인 이전(honsallim) + 1인 살림 콘텐츠 전략 + ★키워드 실측 방법론 [확정] — 세션 #20 신규

- **Q1. 도메인 honsalim.com → honsallim.com 이전 [확정 #20, 실측·주인 구매]**: AliExpress 제휴 채널 등록이 honsalim.com을 **영구 거부**(URL에 'ali' 문자열 포함 — 자동검증+사람지원 모두 거부, 스크린샷 확인). 'ali'는 TLD가 아니라 **단어 "honsalim"** 자체 문제(h-o-n-s-**a-l-i**-m) → .net/.kr 등 TLD 변경 무의미. 해법=**honsallim.com**(겹ㄹ, 살림=sallim 표준 로마자)=‘ali’ 문자열 없음·브랜드 유지. 알리 폼에 honsallim.com 입력 시 'ali' 차단 통과 확인(형식오류로만 진행) → **구매 완료**(Cloudflare Registrar, 만료 2027-06-01, Auto Renew). 코드 이전: SITE_ORIGIN·sitemap·robots·JSON-LD·canonical·/go/·wrangler zone/route·prompt·화면로고(HONSALIM→HONSALLIM)·링크검증기(honsallim 추가, 구 honsalim 전환기 유지). **내부 리소스명**(Pages 프로젝트 `honsalim`·D1 `honsalim-clicks`·DB/로그 파일명)은 **불변**(실제 CF 리소스·알리 비노출). 회귀 623. ※**미배포** — honsalim.com은 #13 옛 사이트 유지. 다음: CF Pages 커스텀도메인 연결 + 301(honsalim→honsallim) + 배포.
- **Q2. 콘텐츠 성장 전략 = 세분화 × 유형 × 갱신 [확정 #20, 조사 근거]**: 넓은 묶음("주방용품" 1페이지) ❌ → ①**세분화**(대표 품목별 카테고리=검색의도 일치) ②**Pillar+Cluster**(구매가이드 pillar + 카테고리 cluster, 내부링크로 토픽권위) ③**살림 전영역 로드맵**(주방·세탁·수납·욕실·냉난방…) ④**★갱신 엔진**(기존 페이지 재수집·재선정 신선도 — 카테고리 유한해도 갱신은 무한, 신규 대비 비용 60%↓). 가드레일: 페이지마다 **진짜 고유 데이터**·점진 확장·품질 미달 미발행 → Helpful Content 패널티(1순위 함정) 회피.
- **Q3. ★키워드 = 네이버 검색광고 실측 기반 (상상 금지) [확정 #20, 라이브 실측]**: 카테고리 선정은 반드시 `collector.naver_searchad`(naver_searchad.env) 실측으로. 핵심 발견: **"1인 ○○"·"자취 ○○"는 검색량 빈약**(1인냄비 350·자취냄비 90·1인식기 50·자취방빨래건조대 70) → 상상 양산 금지. 진짜 볼륨은 **대표어**(빨래건조대 63,120·도마 40,270·행거 68,500·수납장 46,600·미니제습기 18,170, 전부 경쟁도 중간). → **카테고리=대표어, 1인은 콘텐츠 앵글+보조키워드**(원룸·미니·접이식·소형 — seo_keywords 대표+보조 구조에 정합). 의도 불일치 회피: 전기포트·가습기=육아/분유, 그릇세트=혼수/프리미엄.
- **Q4. 채널 적합도 = 가전은 쿠팡·비가전은 알리 [확정 #20, 실측 추론]**: 고볼륨 살림 가전(제습기·인덕션·밥솥) 연관어가 **국내 브랜드 점령**(위닉스·삼성·LG·쿠쿠) → 한국인은 가전을 국내브랜드·국내배송 구매 → 알리 가전 판매 부적합[추정: 220V·A/S·신뢰]. 알리 적합=**비가전·소형·쿡웨어**(행거·빨래건조대·수납·도마·욕실선반·미니제습기). 본격 가전은 쿠팡(Phase 4) 몫 → 살림 본격화 시 쿠팡 가속 신호.
- **Q5. ★카테고리 정의 = 신중·검증 루프 [확정 #20, 주인 강조]**: 카테고리를 잘못 짜면 페이지 전체를 바꿔야 할 수 있는 중요 과정 → **정밀·치밀**. 절차: (a)네이버 실측(대표어·볼륨·경쟁도) (b)`category_sources.yml` 정의(require_any/require_all·exclude·티어 밴드·영문 q) (c)`collect-category --no-dry-run` 라이브 (d)**관련성 제품명 검증** (e)오염 시 **제외어 보강·재수집(멱등 정합화)** (f)깨끗해질 때까지 반복. 도마 실증: 45→25(정리대·랙·걸이·도포기·혼수/접대용 제거, 24/25 정상). status=draft·운영자 승인(E7) 유지.
- **Q6. 홈 카테고리 우선 재편 + 실이미지 [확정 #20]**: 홈을 시나리오 카드 → **카테고리 카드**(category_groups·개념이미지+제품수)로 전환. `image_block` 매크로 실이미지(src) 지원·`category_card` 매크로·`_load_categories_index` concept_image 노출. 히어로·About 개념이미지 재사용. 시즌 회색→**계절 컬러 타일**(봄#8FBE74·여름#7BB8DE·가을#DD9F62·겨울#8FA1B5). ★히어로가 카드와 같은 사진 겹침 → 다음: 신규 배너 이미지로 차별화(주인 지시).

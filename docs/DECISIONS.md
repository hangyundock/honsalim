...
## U. 쿠팡 채널 통합·부트스트랩 [확정] — 세션 #25 신규

> S안(채널별 최선 추천)의 실제 구현. 상세 EVENTS #25. [[assist-not-overstep]]·[[no-speculation]].

- **U1. 쿠팡 채널 = `collector.coupang` 수동 부트스트랩 [확정 #25]**: 쿠팡 Open API는 **최종승인 후에만** 열려 승인 전엔 닭-달걀 → 운영자가 파트너스 '상품 링크 → 블로그용(a태그) HTML'을 `coupang_products.yml`에 기록 → `collect-coupang`(CLI)이 products(`source='coupang'`, slug `cp-<코드>`)+category_products로 적재(정합화 prune 포함). 렌더러=알리 추천6선/카탈로그와 **분리된 "쿠팡 로켓배송" 이미지 그리드**(채널별 최선·S1). 고지 채널인식(쿠팡 상품 있으면 "쿠팡 파트너스 및 AliExpress"). `/go/`는 채널 무관 작동. **가드(§0 재발방지)**: 알리 재수집(category_collect)·재빌드(select_featured 리셋)를 `source='aliexpress'`로 한정 → 쿠팡 링크 보존. 모니터암 파일럿=쿠팡 15개 이미지 그리드 라이브검증. 회귀 678→710.
- **U2. 최종승인 문턱 = 판매금액 15만원(내 수익금 아님) [확인됨 #25, 복수 출처]**: 누적 **판매금액**(고객이 내 링크로 산 상품 결제 총액) 15만원 달성 시 자동 최종승인 검토. 내 수익(커미션 ~3%)이 아님 → 판매 15만원이면 내 수익은 ~4,500원. 승인 후에야 쿠팡 API(상품·이미지·딥링크 자동수집)→`collector.coupang` API화로 **전 카테고리 자동화**. 그 전까진 수동 부트스트랩 최소 + 트래픽/판매로 15만원 도달이 과제(낮은 문턱이나 트래픽이 진짜 병목).
- **U3. 브라우저 자동화(claude-in-chrome) = 쿠팡 정책 차단 [확인됨 #25]**: 확장 설치·연결·사이트 제한까지 됐으나 `partners.coupang.com` 이동 시 "blocked by your organization's policy"(상거래·금융 사이트 차단 추정) → **쿠팡 링크 생성 자동화 불가**. 다음 세션 재시도 금지. 링크 생성은 운영자 수동(블로그용 HTML 붙여넣기)이 현 유일 경로. ★교훈([[no-speculation]]): 미확인을 사실처럼 전제하고 주인에게 셋업시켰다가 실패 — 치명적 가정 먼저 확인/경고.
- **U4. 배너 비차단 — 위치×맥락이 효과 결정·측정 시스템 안 만듦 [확정 #25, 주인 결정]**: 이전에 "배너 안 됨/규정 위반"으로 주인 결정을 막은 것 = 월권([[assist-not-overstep]]). 리서치 결과: 광고는 **형태보다 위치가 먼저**(ATF 고가치=CPM ~40%↑로 시장 교차검증), **본문 맥락형이 일반 배너보다 전환↑**(배너 블라인드니스). **메인 첫 화면 쿠팡 배너(주인 원안)는 허용** + 카테고리 결정지점(추천·비교 직후) 배치. **효과 검증=쿠팡 수익 리포트**(자리 몇 군데·트래픽 적어 별도 클릭 측정 인프라는 과잉=안 만듦). 다음 작업=배치 구현.
- **U5. §9 함정3 보강 — 쿠팡 공식 제공 이미지 hotlink = 정식 [확정 #25]**: 금지는 "쿠팡 product 페이지 이미지를 **다운로드·자체호스팅**"만. 쿠팡 '블로그용 HTML'이 제공하는 `image*.coupangcdn.com/image/affiliate/banner/...` URL을 **hotlink(임베드)는 정식 사용** → 카드 이미지 그리드 가능. ⚠️단 광고차단기(uBlock 등)가 'affiliate/banner' URL을 차단 → 광고차단 사용자(주로 데스크톱)는 이미지 미표시, **graceful fallback**(로켓배송 박스). 대다수(모바일·비광고차단)는 표시. 자체호스팅 우회는 §9/계정위험으로 안 함(프록시는 회색지대).

...
- **V5. 카테고리 흡수로 비공개된 글 = 301 리다이렉트 [확정 #31]**: 흡수돼 unpublish된 글의 라이브 URL은 `renderer.REDIRECTS`→`build/site/_redirects`(Cloudflare Pages 301)로 후속 카테고리 영구 이전(404·SEO 손실 방지). ~~게이밍의자 글 `/articles/kw-e3d08a2c/`→`/categories/office-chair/`~~ **(#44 폐기·재발행 — BB3 참조. slug 결정적이라 재발행 글을 좀비화. 원칙 자체는 유지=노트북받침대 `kw-4d525971`).**
...
## Z. 무인 발행 자가복원·재고 알림·텔레그램 [확정] — 세션 #41 신규

> 주인 "미리 세팅한 쿠팡 키워드가 무인 발행 안 됨" 지적 → 6하위시스템 정밀조사(DB+코드)로 '게이트 반려 글이 조용히 죽고 화면에 안 보이는' §0 위반 근본수정. 상세 EVENTS #41. [[autonomous-safe-system]]·[[autopublish-operational-model]].

- **Z1. 게이트 반려 = 자가복원(자동 재시도)+가시화, 조용한 데드엔드 금지 [확정 #41, 근본수정]**: 무인 사이클이 생성한 글이 게이트 반려되면 옛 코드는 키워드를 `status='drafted'`로 두는데 `auto_pick_keyword`는 `pending`만 집어 **영영 재선정 안 됨**(재시도 0·`fail_count` 미갱신·digest 미집계·대시보드 "글 생성됨"으로 표시=정상처럼 보임). 실측: 주인이 쿠팡 배너 저장한 7키워드 중 3개(등받이·메쉬·허리편한의자)가 이렇게 방치. 근본수정=①반려 시 `fail_count`++ → 상한(`keyword_max_gate_retries` 기본3) 미만이면 **자동 `pending` 복귀**(다음 사이클 재생성)·상한 도달 시 `failed`로 격리. ②digest에 `queue_retrying`·`queue_gate_failed`·`gate_failed_keywords` 추가 + 상한 도달 시 항상 [ALERT]. ③대시보드 카운터 3종(반려/재시도/미발행) + 키워드 표 '사유' 컬럼(반려는 "글 생성됨" 아닌 실제 상태 표시). 라이브 실증: 07-03 사이클이 '서재책상'을 자동 생성·반려·`fail_count=1`로 자동 재시도 복귀(설계대로 첫 작동).
- **Z2. 재생성 피드백 = 실행지시 변환 (게이트 약화 금지) [확정 #41, 근본수정]**: `headings_keyword_low`(소제목에 대표키워드 0개) 반려 시 옛 코드는 에러 문자열을 그대로 LLM에 되먹여(`feedback=[raw issues]`) 재시도해도 LLM이 소제목에 키워드를 안 넣어 2회 후 반려(허리편한·게이밍책상). SEO 게이트 자체는 정상(`_ns` 공백무시 매칭이라 "허리 편한 의자"도 인정) → **게이트가 아니라 생성 지시가 문제**. 근본수정=반려 사유를 실행가능 지시로 변환(예 headings_keyword_low → "소제목(##) 중 최소 1개를 '<키워드>'로 시작하도록 작성"). 게이트 기준은 절대 안 낮춤(§0·C17 정신).
- **Z3. 반려 재시도 원클릭 (대시보드 버튼 + 더블클릭 런처) [확정 #41, 비개발자 편의]**: CLI `keyword-requeue`(게이트 반려만·수동반려/발행글 연결 키워드 제외·쿠팡 배너 보존·`fail_count` 리셋)를 두 경로로 노출 — ①대시보드 키워드 탭 **[🔁 반려 재시도]** 버튼(행 선택=1개·미선택=일괄) ②`scripts/update_and_requeue.bat`(더블클릭=git pull + requeue·순수 bat·기존 cleanup 런처 패턴). 안전가드 2종: `--id`는 발행 글 있으면 차단(중복 글 방지), 일괄은 **최신** draft가 게이트 반려인 키워드만(과거 반려 이력으로 발행성공 키워드 재트리거 방지).
- **Z4. 대시보드 3줄 안내 배너 + 텔레그램 푸시 채널 [확정 #41, 주인 지시=naver_blog UX 미러 + #39 이월 완성]**: ①**3줄 배너**(`queries.auto_forecast`/`banner_lines`·PyQt 비의존): 상태(무인 ON/OFF)·예정(쿠팡 첨부 재고로 언제까지·날짜+키워드 소비순서)·다음 할 일(심각도별 행동지시). 심각도 ok/caution/alert 배너색. ②**텔레그램**(`common/notify.py`·표준 urllib·의존성 0): 무인 사이클 종료 시 일일 리포트(**하트비트 겸용**—예약 후 안 오면 스케줄러/PC 죽음 신호) + 경보(쿠팡 첨부 소진 임박(기본 ≤2)/0·게이트 반려 상한·발행 0 위험·발행 실패). 일일 리포트 OFF여도 경보는 발송(fail-loud). **secrets = `D:\secrets\affiliate_hub\telegram.env`**(config.load_secrets가 *.env 자동 로드) — **TIMA(`D:\secrets\settings.json`)의 봇/챗ID 재사용**(주인 지시·happyending 기존 텔레그램 그대로). 미설정 시 무동작·모든 발송 실패는 사이클에 무영향(§0 격리). 설정창에 재시도상한·쿠팡부족기준·일일리포트 3키 + 설정탭 [📨 텔레그램 테스트 발송]. 라이브 검증: telegram.env 생성→실발송 성공.

## AA. 무인 발행 글 제품 구색·좀비 페이지 차단·견고화·발행 알림 [확정] — 세션 #42 신규

> 주인이 오늘 무인 발행된 메쉬의자 글 사진과 함께 "제품 수량이 수동 시절 대비 부족(3개뿐)·어디서도 이 글로 가는 링크가 없어 좀비 페이지" 지적. 최우선 Google SEO, 다음 소비자 실활용 정보·제품 구색. 상세 EVENTS #42.

- **AA1. 키워드 글 전체 카탈로그 = 글 상품(앞) + 매핑 카테고리 광폭 구색(뒤) [확정 #42, 근본수정·#38 과교정 정정]**: #38이 'base 카테고리 통째 상속'을 '글 고유 데이터'로 바로잡을 때 **전체 카탈로그까지 글 수집분(키워드 적합 3~8개)으로 좁혀** 무인 발행 글이 '고를 게 없는' 페이지가 됨(라이브 적발: 메쉬의자 글 전체 3개 vs 참조 카테고리 23~46개). 추천 픽·쿠팡존·비교표·데이터 요약은 #38대로 **글 고유 유지**(키워드 적합·진실성), 소비자가 훑어 고르는 '전체 보기'만 매핑 카테고리 전체 구색으로 확장(같은 slug 중복 제거·글 상품 우선). 타입 필터 칩 복원. `_article_as_category_ctx`. 라이브 실증: 메쉬의자 3→24개·모니터받침대 11→41개.
- **AA2. 좀비 페이지 차단 = 카테고리 인덱스 세부 가이드 칩 + 글 빵부스러기(카테고리 계층) [확정 #42, 근본수정]**: 상단 메뉴 '카테고리' 인덱스에서 발행 글 링크가 0개라 카테고리 탐색 경로에서 새 글이 고아(주인 적발). ①카테고리 인덱스 카드 하단에 **세부 가이드 칩**(매핑 발행 글=키워드 라벨, cap 4·초과 '+N개', 카드 `<a>`→`<div>`+`cc-main`으로 재구조화, `_attach_guides_to_groups`) — 대분류→카테고리→키워드 글 3층 완성. ②글 빵부스러기 = **홈 › 카테고리 › {카테고리} › 글**(화면 crumb + JSON-LD BreadcrumbList 동시, 옛 '내맘대로 세팅' 고정 폐기 — 시나리오 허브 오소속 신호 제거). 토픽 클러스터(허브↔스포크 양방향)로 소카테고리 신설 대신 얇은 중복 페이지 회피. 렌더 파생이라 새 글 발행 시 자동 반영(무인). 적대검증 2인 → soon 카드 CSS 잠복결함 1건 수정.
- **AA3. 무인 견고화 3종 [확정 #42, 라이브 적발·근본수정]**: ①**OpenRouter 제공자 풀 403 재시도**: 라우팅된 특정 제공자(DigitalOcean)의 일시 403('Provider returned error')으로 22:18(1차) 무인 사이클 0편·쿠팡 키워드 격리 → 429와 동일 재시도(다른 제공자로 라우팅). 진짜 인증 403(키 폐기)은 fail-fast 유지. ②**생성 예외 자가복원**: 1회 예외로 영구 `failed` 격리하던 것을 게이트 반려와 동일한 `fail_count`·상한(#41 Z1)으로 통일 — 상한 미만 pending 복귀·자동 재시도. ③**키워드 탭 UX**(주인 지적 "발행 순서·리스트 기준을 알 수 없다"): '발행 예정' 컬럼(①7/7(화) 식·`auto_forecast` 재사용=배너와 단일 소스)·발행 예정 순 정렬·용어 정리(탭 "키워드"→"제품 키워드", "카테고리·모니터링"→"카테고리(제품 묶음)"+안내문: 카테고리=제품 키워드 글이 소속되는 묶음 페이지·새 글은 여기 등록 안 됨·세부 가이드로 자동 연결).
- **AA4. 텔레그램 발행 알림 = 발행 글 제목+URL 푸시 [확정 #42, 주인 지시]**: 무인 발행 완료 시 텔레그램으로 **발행된 글 제목 + 라이브 URL**(핸드폰 탭→페이지 확인). 발행 전후 published 글 id 집합 비교로 **이번 회차 발행분만** 정확히(race-free). 발행이 있으면 일일 리포트 OFF여도 발송(주인이 원한 발행 결과 알림). 경보와 한 메시지. Z4 `_auto_cycle_notify` 확장(`_published_article_ids`/`_newly_published_articles`). 라이브 검증: 실발송 성공. ※발행 자체는 auto-cycle 경로만 알림(반자동 publish-queue·수동 대시보드 발행은 별도).

## BB. 무인 침묵 정지 복구·안전정지 경보·발행 글 title·게이밍의자 재발행 [확정] — 세션 #44 신규

> `/honsalim-start` 브리핑 중 무인 발행이 07-08~17 **10일간 조용히 정지**한 것을 적발(원인=#43 가이드 작업이 운영 폴더를 detached HEAD로 이탈시켜 auto-cycle 래퍼의 `main 아님` 가드가 매일 안전 정지). 주인 승인으로 복구+재발방지+주인 지시 3건 처리. 상세 EVENTS #44.

- **BB1. 무인 래퍼 안전 정지 = 텔레그램 경보(fail-loud) [확정 #44, 근본수정·§0]**: `run_auto_cycle.ps1`이 `main 아님`·`git 불가`·`DB 없음`으로 안전 정지(exit 0)할 때 **아무 알림이 없어** 며칠씩 방치되던 침묵 정지를 fail-loud로 전환. `cli notify-alert <msg>`(secrets 로드→telegram, **항상 exit 0** — 미설정·발송 실패·예외 어떤 경우든 래퍼를 절대 안 막음) + 래퍼 3개 정지지점에서 `Send-Alert`. 자가복원 3겹 완성(#41 게이트 반려 + #42 생성 예외 + #44 안전 정지). ★운영 폴더는 반드시 `main` 브랜치 유지 — /honsalim-start서 확인. 메모리 [[autonomous-detached-head-silent-stop]].
- **BB2. 발행 글 `<title>` = 글 제목 (카테고리 guide_title 상속 폐기) [확정 #44, 근본수정·SEO]**: 카테고리 매핑 글은 category.html 재사용(#34)이라 `<title>` 블록이 카테고리 `guide_title`을 상속 → 같은 카테고리 발행 글 전부가 **동일 `<title>`**(중복 제목·검색결과 제목 불일치·라이브 적발: 서재책상 글 title="컴퓨터 책상 고르는 법…")·og:title·JSON-LD headline(#40 IDX-03)과 어긋남. → category.html title 블록을 `is_article`이면 글 제목(`category.name`, 글 렌더 시 글 제목으로 덮임), 카테고리 페이지는 기존 그대로(이미 색인된 제목 불변). 회귀 가드 2종.
- **BB3. 게이밍의자 글 재발행·301 폐기 / 노트북받침대 301 유지 [확정 #44, 주인 결정]**: #31의 `kw-e3d08a2c(게이밍의자) → /categories/office-chair/ 301`은 게이밍의자 상품을 '의자' 타입으로 흡수하며 글을 비공개한 것. 키워드 글이 카테고리와 **공존**하는 현행 모델(#39~)에서 이 301은 승인 콘텐츠를 좀비화(slug=sha1(키워드) **결정적**이라 리다이렉트가 남으면 재발행 글 URL 영구 가로채임) → `REDIRECTS`에서 제거 + `republish-article`. 라이브 200·글 제목 정상. **노트북받침대 `kw-4d525971` 301은 유지**(모니터받침대 카테고리와 상품 100% 중복 + 같은 주제 글 이미 라이브 → 재발행 시 3중 중복·Scaled Content 위험). ※게이밍의자 글은 매핑 미해당이라 article.html 폴백 렌더(빵부스러기 '내맘대로 세팅')로 남음 — 신규 글식 카테고리 구조 이관은 선택 과제(#45).

## CC. 추천 키워드 다양화·색인 성장(E-E-A-T·IndexNow)·적대검증 [확정] — 세션 #45 신규

> 주인이 대시보드 추천 다이얼로그 사진과 함께 "추천 키워드가 의자·컴퓨터에만 집중, 폭넓게 + 구글·네이버 SEO 최적화"를 지시. 원인=추천 씨앗(seo_keywords.yml)이 책상 클러스터 5개뿐. 비판 3인 검증 → 근본수정 + 색인 성장(E-E-A-T·IndexNow) + 적대검증 3워크플로우 반영 + 여름이불 발행. 상세 EVENTS #45.

- **CC1. 씨앗 확장(5→7) + require_terms 니치 필터 + 교차중복 정합 가드 [확정 #45, 근본수정]**: 공개 카테고리인데 씨앗 0이던 **도마·미니밥솥**을 네이버 검색광고 실측(`build_entry`)으로 생성·반영(브랜드·카테고리 제외어 미러). ①**require_terms**(니치 한정어) 신설(`keyword_research`): '밥솥' 연관어는 압력·전기밥솥 헤드(4만)·6인용이 지배 → 미니·1인·2인·소형·자취만 채택(세그먼트 이탈 오염 구조 차단). seo_keywords.yml→default_seeds→recommend 배선. ②**교차중복 정리**: monitor-stand secondary의 '모니터암'·'모니터 거치대'·'노트북 받침대'가 monitor-arm·laptop-stand를 first-match로 가려 오매핑하던 것 제거. ③**`seo_keywords.lint_alignment` + doctor §15**: 드리프트(씨앗∉category_sources)·교차중복·공개 카테고리 씨앗 누락을 FAIL/WARN으로 상시 가드(mini-rice-cooker처럼 프로비저닝 후 씨앗 누락 재발 방지). 라이브 실측: 추천 30건이 7카테고리로 분산(주방 8·나무도마 1위).
- **CC2. require-empty(비전 전담) 카테고리 키워드 글 = 검증 카탈로그 재사용 [확정 #45, 근본수정·오염 봉인]**: `mini-rice-cooker`는 `require_any=[]`(관련성=수집 시 비전 게이트 전담·#36)라 키워드 글 경로의 `filter_products`가 사실상 무필터(전량 통과) → fresh 알리 검색 오염 상품이 완전무인(min_published=0) 발행까지 직행하는 구멍. `_gather_keyword_candidates`에서 require 없는 카테고리는 fresh 검색 대신 **비전 검증된 category_products 카탈로그를 재사용**(쿠팡존 제외·상한 page_size×2). 카탈로그 비면 후보 0=fail-closed(빈 글 차단 #38). 상품 0 사유를 note에 보존(digest 가시화).
- **CC3. 무인 fail-loud 보강 = refill 매핑·공개 한정 + 생성0 경보 + category_draft [확정 #45, §0]**: ①**refill 가드**: `auto_pick_keyword`의 추천 자동보충이 미매핑·비공개(draft) 카테고리 추천을 큐에 넣던 것 차단(첫 '매핑+공개' 후보 선택, 전부 부적격이면 None→생성0→abnormal 경보). **스캔폭 200**(표시용 limit 20과 분리 — 미매핑 후보가 상위 20에 잔류해 21위+ 매핑 후보를 영영 못 집던 래칫 해소·적대검증). ②**digest 생성0 경보**: live 사이클이 생성 목표(target>0)인데 made=0이면 보류·큐 상태와 무관하게 abnormal→[ALERT]·텔레그램(refill 고갈·전건 failed의 '조용한 0편' 포착). ③**`auto_approve` category_draft**: 매핑돼도 카테고리 draft면 보류(공개 허브 없는 고아 글 차단·카테고리 공개 시 자동 해소). ④**`publishability(conn)`**: draft 카테고리를 'category_draft'로 판정 — 대시보드 발행 예측·digest 집계·auto_pick 강등이 실제 보류와 단일 소스(생성 비용 후 보류 어긋남 방지).
- **CC4. E-E-A-T = 운영자 Person Schema + 저자 통일 + 정직성 정비 + /privacy/ [확정 #45, M2 구현·색인 신뢰]**: ①**운영자 Person JSON-LD**(`build_person_jsonld`·M2): 필명 혼살다·운영철학·knowsAbout·/about/·**사진 없음(M2-5)** → About 페이지 주입. Article author를 **'혼살다'+url(/about/)**로 통일(옛 '혼살림 운영자'가 byline 혼살다와 불일치하던 것 해소·전 글 렌더 재생성 소급). ②**정직성 정비(L3·QRG §4.5.3·§0)**: about·method의 1인칭·거짓 경험 문구("실제 경험에서 출발"·"검색에 며칠"·"실사용 경험")와 완전무인 모순 문구("사람이 검토한 뒤 공개") 제거 → 데이터 기반·자동 검증+사후 관리로 재서술. ③**/privacy/ 신설**(PIPA E2·§30 필수항목: 수집없음·위탁(Cloudflare)·제휴이동·문의처리·정보주체 권리·안전성 확보·보호책임자) — 실수집 현실 기준(직접 수집 없음·D1 로깅 보류 R4). footer·about '빈 약속' 앵커(/about/#privacy)→실페이지. 라이브 curl 전부 검증.
- **CC5. IndexNow 전체 구현 [확정 #45, F4 이월 완성·적대검증 견고화]**: `deployer/indexnow.py`(never-raise §0·urllib). ①**`<key>.txt` 빌드 자동 생성**(env·형식 검증·공개키) — 모든 빌드 진입점(refresh_cycle·**cmd_build --full**)에서 secrets 로드해 경로별 키파일 유무 비대칭(삭제 커밋→라이브 404→통지 무효화) 차단. ②**배포 성공 후 변경분만 통지**: 직전 배포 sitemap diff(추가·lastmod 변경·삭제) + 갱신 카테고리(전체 재제출 반복은 IndexNow 지침 위반·호스트 신뢰 하락). ③**발송 전 라이브 keyLocation 폴링**(200+내용 일치, CI 시차·키 유실 가드) — 미확인 시 생략(다음 배포 재시도). ④doctor §16. ★**Google 무관**(IndexNow 미참여·Indexing API 금지 F6 유지). 라이브 검증: 키파일 200+일치.
- **CC6. 여름이불 발행 = 주인 결정(as-is) / 침구 카테고리 미구축 [확정 #45, 주인 결정·과잉설계 회피]**: 07-18 무인이 여름이불(주인 쿠팡 배너 3개 첨부)을 생성·validated했으나 **미매핑으로 자동승인 보류**(=07-18 발행 0의 원인·경보는 정상 발화). 주인 결정=**지금 발행**(수동 게이트 승인→글 #13 `kw-d1e641be`·라이브 검증: 제목·대가성 고지·쿠팡 3개만·수익링크 정상). **침구 카테고리는 구축 안 함**(글 12편 규모에 과잉설계·[[feedback_assist_not_overstep]]) — ★**침구는 쿠팡 전용**(AliExpress bedding 검색=애니/캐릭터만·`search_keywords.yml` 라이브 검증 기존). 정직 고지: 미매핑=독립 페이지(허브·내부링크 약함)·신생 페이지 색인 리드타임상 올여름 트래픽은 제한적. 침구를 지속 콘텐츠로 원하면 그때 쿠팡 카테고리 검토.

## DD. 색인 원인 진단(크롤 예산)·카테고리 sitemap lastmod·무인 여름이불 자동비공개 [확정] — 세션 #46 신규

> #45 최우선 관찰(무인 다양화·IndexNow)을 라이브 검증(성공)한 뒤 주인 "성장 추천" 요청 → 색인 커버리지 GSC 실데이터 심층 진단 → 근본수정(lastmod) + 무인 자동비공개 적발. 상세 EVENTS #46. [[growth-first-priority]].

- **DD1. 색인 격차 원인 = 크롤 예산/권위(어린 사이트)이지 기술 결함 아님 [확정 #46, GSC 실데이터·정량]**: honsallim.com 라이브 29 URL 중 색인 4. GSC '색인 안 됨' 분류=①robots.txt 차단 16=**/go/ 제휴 이동링크**(`rel="sponsored nofollow"`·의도적 차단·정상) ②**발견됨-미색인 11**(카테고리 5·정적 about/method/guides/categories·페르소나 — URL검사 '최근 크롤링 해당없음'=**미크롤**=크롤 예산) ③크롤됨-미색인 2 ④404 1 ⑤리디렉트 1. 정량 반박(코드+라이브): 카테고리 얇음 아님(본문 2,100~2,400단어)·카테고리 상호중복 아님(8-shingle Jaccard 평균 3.3%)·내부링크 튼튼(미색인 페이지 17~30 소스페이지+홈 링크)·noindex/canonical/X-Robots 전부 클린·내부 깨진링크 0(3,843 검사). → **색인을 강제하는 코드 수정은 없음**. 유일 직접 레버=주인 GSC URL검사 색인 요청(11개 완료)·간접=발행량·권위·시간. 404는 우리 내부링크 아님(외부/과거 URL·방치 무해). ★글↔매핑 카테고리 8-shingle 겹침 44~50%는 향후 크롤 후 '둘 중 하나 색인' 리스크로 관찰(현재는 카테고리 미크롤이라 블로커 아님).
- **DD2. 카테고리 sitemap lastmod = 연결 상품 MAX(last_seen_at) [확정 #46, 근본수정·#40 SITEMAP-02 보완]**: 카테고리·정적은 lastmod가 없어(원설계=부정확 lastmod 금지·Google 가이드) 재크롤 힌트·IndexNow sitemap-diff 신호가 없던 갭. 카테고리는 **정확한 변경일 보유**(연결 상품 `MAX(last_seen_at)`=이미 페이지에 '데이터 수집일'로 쓰이던 `data_summary.collected`) → sitemap 카테고리 lastmod에 연결(`renderer._sitemap`·`cat_lastmod`). **정적·페르소나는 정확한 변경일 없어 생략 유지**(원설계·날조 회피). 부수효과: IndexNow `deploy_urls` 변경분 감지가 카테고리도 diff로 포착(우회 refreshed-slug는 같은날 재수집 안전망으로 유지·주석 정합). 재발방지 가드 2종(lastmod==last_seen_at 정확성 / 정적 무lastmod). 실데이터 검증: 공개 6 카테고리 상이 날짜(06-26~07-20). 회귀 1114→1116. 파일=renderer.py·indexnow.py·test_renderer.py(커밋 완료·배포는 운영 코드 동기화 후 빌드). ★마진 개선(색인 은탄환 아님·DD1).
- **DD3. 여름이불 무인 자동 비공개 = fail-closed 사후 모니터 [관찰 #46]**: #45 주인이 수동 발행한 여름이불(id 13 `kw-d1e641be`·미매핑·쿠팡3)을 07-20 11:11(02:11 UTC) auto-cycle의 `article_guardrail.monitor(auto_unpublish=live)`가 '미달' 판정→**자동 비공개**(로그 "미달 1편·자동 비공개 1편"·published_at NULL). §0 fail-closed 정상 작동이나 **주인의 #45 발행 결정을 되돌림**([추정] 원인=미매핑·추천 데이터 얇음). 발행 글 12편(도마 +1·여름이불 −1 상쇄). → 주인 결정 필요(방치/개선 후 재발행/침구 쿠팡 카테고리). ※C6·C21 사후 모니터 자동 비공개 설계대로 동작.

## EE. 무인 발행 3일 공백 근본수정 + 전체 정밀진단 안정화 [확정] — 세션 #47 신규

> 세션 시작 브리핑에서 **07-21~23 발행 0편 3일**을 적발 → 로그·코드 실측 진단 → 근본수정 2건. 이후 주인 지시로 **Fable 5 전체 정밀진단**(무인 핵심 경로 전수) → 비판 재검증 통과분 5건 수정. 상세 EVENTS #47. [[autonomous-safe-system]].

- **EE1. 3일 공백 원인 = 한 키워드의 게이트 반려 3연속(침묵 정지 아님) [확정 #47, auto_cycle.log 실측]**: 07-21·22·23 모두 스케줄러 정상 기동(exit=0). 키워드 #25 `스텐도마`(도마·4자)가 3일 연속 seo 게이트 반려 → `keyword_max_gate_retries=3` 상한 도달 → `failed` 격리. 07-24 다른 키워드(`도마추천`)로 자동 전환돼 발행 재개(이후 6일 연속 정상). 사이클 내 재생성(`seo_max_attempts=2`)까지 **6회 시도 전부 실패**. 밀도 추이: 0.44→0.41(피드백 무효)·0.00→0.54(+truth 1인칭)·0.28→**3.94%**(3회→약 37회 오버슈트로 상한 3.5% 위반). ★게이트는 무죄(3.94%는 실제 도배) — 생성기가 유죄.
- **EE2. 게이트 반려를 '생성 성공'으로 집계하던 결함 = fail-loud 관통 [확정 #47, 근본수정]**: `cmd_keyword_generate`가 게이트 반려에도 `return 0` → auto-cycle `if rc == 0: made += 1` → `made=1`. 그 결과 #45가 넣은 가드 `(target > 0 and made == 0)`이 False가 돼 **`abnormal=False` → 07-21·22 이틀 텔레그램 무경보**(3일차 상한 도달 경보로 뒤늦게 노출). `RC_GATE_REJECTED=4` 신설로 산출 0을 성공과 분리(0=draft 산출/2=대상 없음/3=상품 0·빈 글/4=게이트 반려). 키워드 상태 전이(pending 복귀·failed 격리)는 불변. 부수 개선: 대시보드가 nonzero를 '완료(경고)'로 표시해 반려 가시성↑. 가드=반려 시 `made==0`·`abnormal==True` + 통과 대조군.
- **EE3. 밀도 재생성 지시 = '총 약 N회' 절대 목표(차이 아님) [확정 #47, 근본수정·Z2 보완]**: 옛 지시는 "목표(~1.7%)로 높이세요"라는 정성 표현(현재 밀도 %는 `(원인: …)`로 이미 전달되고 있었음 — 초기 진단 오류 정정)이라 LLM이 **필요 반복 횟수를 역산 못 해** 진동. `_density_directive`가 밀도식(`키워드길이*반복수/산문길이`)을 역산해 목표 횟수 계산(카테고리 floor/ceil 오버라이드 시 범위 중앙으로 클램프) + "약 N자마다 1회꼴" 밀도 감각 제공. ★**재생성은 직전 본문 없이 처음부터 다시 쓴다**(`claude_client.build_user_prompt`=원본 템플릿+보완 지시만·이전 body_md 미전달) → "N회를 더"는 기준 본문이 없어 성립 불가 → 반드시 절대 목표. 실측 재현: 4회·37회 양방향이 동일 목표(16회)로 수렴. metrics 부재 시 옛 지시 폴백. `validator.seo` metrics에 `primary_len`·`density_floor`·`density_ceil` 추가. ★게이트 기준 불변.
- **EE4. 비공개-only 날 라이브 미반영 = 가드레일 무력화 [확정 #47, 근본수정·U/W #30 결함 잔존 분기]**: 발행 0 + 가드레일 자동비공개만 있는 날 auto-cycle이 `cmd_build`+`cmd_deploy`를 탔는데, `git_push`는 **"commit 안 함, push만" stub**이라 재빌드된 build/site가 커밋되지 않아 CI 미발동 → **'미달' 판정으로 내린 글이 라이브에 계속 노출**(다음 발행 성공일의 refresh_cycle 커밋이 쓸어갈 때까지 — 발행 공백기와 겹치면 며칠). 07-20 여름이불은 같은 날 나무도마 발행이 있어 우연히 반영된 것. 발행 경로(#32 W)와 동일하게 `refresh_cycle`(build+DEPLOY_PATHS commit+push) 재사용으로 교체 + 실패 rc를 텔레그램 경보로 전달(옛 코드는 `None`으로 삼킴). `_deploy_ns` 죽은 헬퍼 제거(stub 경로 재사용 유혹 차단). 가드=옛 경로 호출 시 즉시 실패하는 지뢰 테스트 + 실패 경보 + 무변경 정상종료 3건.
- **EE5. 무인 래퍼 견고화 3건 [확정 #47, 라이브 실증 포함]**: ①**크래시 무경보 해소** — `cli`가 예외로 죽으면(exit≠0) digest·텔레그램이 아예 안 나가 침묵(가드 3종만 경보하던 #44 잔여 구멍) → `exit≠0`에 `Send-Alert`(cli 자체 경보와 중복 가능하나 침묵보다 중복 우선). ②**git pull 판정 근본수정** — `$ErrorActionPreference="Stop"` + `2>&1` 조합에서 git의 정상 진행 메시지(stderr)가 예외로 승격돼 **07-20~29 매일 "git pull 실패"로 기록**·성패 판별 불능(원격 전진 시 자기갱신 침묵 실패 → 다음날 발행 push 비FF 거부 정지 경로). EAP 간섭 제거 + `$LASTEXITCODE` 판정으로 교체 — **라이브 실증**(문제의 stderr 3줄이 데이터로 흐르고 `exit=0` 정상 판정). ③**로그 한글 영구 깨짐** — python UTF-8 출력을 PS가 cp949로 디코드해 `?쒖옉` 형태로 저장(무인 1차 증거의 판독성 훼손·본 세션 진단에서도 지장) → `[Console]::OutputEncoding=UTF8`(콘솔 없는 환경 try/catch). ※옛 로그는 복구 불가.
- **EE6. SQLite `busy_timeout=10000` [확정 #47, 예방]**: PRAGMA 4종에 busy_timeout이 없어 기본 0ms — 주인이 대시보드로 쓰기 작업 중일 때 스케줄러가 **대기 없이 즉시 'database is locked'** 크래시 가능(WAL은 읽기-쓰기만 해소·쓰기-쓰기는 아님). 발생 기록은 없으나 [확인 불가], EE5①과 결합하면 침묵 크래시가 됐다. 10초 대기 추가 + 회귀 단언(≥5000).
- **EE7. 비판 검증 기각 2건 [확정 #47, 기록]**: ①"refresh_cycle push 실패 시 커밋이 고아가 된다" → 다음 배포 push에 자동 합류함을 확인, 기각. ②"auto_pick의 전건 미매핑 폴백이 사이클을 낭비한다" → #39에서 의도적으로 설계·digest 경보로 가시화됨, 기각. ★**"사이클 내 키워드 폴백" 제안도 철회** — LLM 호출 2배 vs 실손실 3/40일(발행률 92.5%)·4일차 자동 복구. 비용 방어 결정(CLAUDE.md §6·seo.py "게이트 과민=결제 비용")과 충돌하는 과잉설계. EE2·EE3로 반려율 자체를 낮추는 게 선행([[assist-not-overstep]]).

## FF. 밀도 벽 돌파(B2 절대 하한)·1인칭 오탐 반전·비용 낭비 방지·slug 근본수정 [확정] — 세션 #48 신규

> 07-30·07-31 발행 0편 적발 → 라이브 6회 반복 실패 후 7회차 성공. 그 과정에서 **프롬프트 층위로는 풀 수 없는 구조적 벽**과 **측정 없이 반복한 낭비**가 함께 드러났다. 상세 EVENTS #48. [[autonomous-safe-system]]·[[critical-final-review]].

- **FF1. 밀도 실패의 정체 = 횟수 미준수가 아니라 '문체 전환' [확정 #48, 저장 본문 전수 확인]**: `책상추천` draft 38·39가 이틀 모두 정확히 31회(4.33%·3.97%). 분포를 보면 소제목 8/10개·상품 문단 전부·FAQ 답변 전부에 1~2회씩 기계 배치하고 키워드를 **상품 지칭 명사**로 사용("가성비 높은 책상추천이다"). 8섹션 표준 구조라 '구조 단위마다 1회'의 총합이 두 날 같은 값으로 수렴한 것 — 우연이 아니다. #47 EE3의 정량 지시(총 약 N회)만으로는 막히지 않았다.
- **FF2. ★지시는 모든 경계에 같은 무게의 '탈락'을 달아야 한다 [확정 #48, 3회 라이브 반증]**: 모델은 **"탈락"이라고 명시한 조건만 추적**한다. 옛 지시는 하한에만 탈락 경고(#19 "하한 미달도 탈락이니 충분히 쓸 것") → 31회 도배. 1차 수정이 상한에만 붙이자 → **7회·소제목 0개**로 반대편 탈락(0.78%). 2차 수정에서 하한·상한·소제목 최소치 3개를 전부 `(필수)`+숫자+탈락 결과로 명시해 소제목은 해결. ★부수: "두 번째 언급부터는 대체어로"가 모델을 **전부 치환**하게 만들어 0.78%까지 떨어뜨린 직접 원인 — "최소 횟수를 채운 뒤, 그래도 과하면 그때만"으로 교정. 8섹션 제목이 system 표준이라 모델이 그대로 복사하므로 **고칠 소제목을 실물로 제시**("## 1. 누구를 위한 {kw} 가이드인가")해야 지켜진다.
- **FF3. ★키워드 글 밀도 하한 = 절대 횟수(min_count>=4), %가 아니다 [확정 #48, 주인 결정 B2·라이브 6개 본문 실측]**: 검색어 복합어('~추천' 접미형)는 도배를 금지하면 자연 사용량이 **4~7회로 고정**되고 본문 길이에 거의 비례하지 않는다(지시 숫자 8~14를 4회 연속 무시). 절대 횟수이므로 **%하한은 어떤 값이어도 긴 글에서 뚫린다** — 같은 세션의 결정 A(하한 1.0→0.8%)도 5,199자 글에서 0.41%로 즉시 뚫려 폐기·대체됐다. `validator.seo`가 `seo.min_count`가 있으면 %하한 대신 절대 횟수를 검사(`count_low` 신설). ★**상한(3.5%)·도배 금지·위치 게이트(제목·도입부·소제목)는 불변** — 완화가 아니라 모델의 실제 행동과 하한의 정합. 하한의 목적("키워드를 사실상 안 쓴 글" 차단)은 위치 게이트가 이미 담보한다. 카테고리 페이지는 일반 명사라 %하한 유지. ※#48 종료 시점 관찰: 본문 예산 축소 후 3~5회로 내려가 여유 축소 — 재시도 3회가 흡수.
- **FF4. 1인칭 게이트 오탐 = 경험 동사 화이트리스트로 반전 [확정 #48, 주인 승인·2차 근본수정]**: `(\d+개월|\d+년) (사용|썼)` 패턴이 **후기 인용·객관 서술**을 잡아 재시도를 소모했다(라이브 3회 중 2회) — "2~3년 사용에도 견고하다는 후기가 많지만", "1~2년 사용 후 이동할 가능성이 높은 환경". 1차 수정(비경험 꼬리 블랙리스트: 에/시/기간…)은 곧바로 '사용 **후**'로 재발 — 한국어의 비경험 꼬리는 열린 집합이라 나열로 끝나지 않는다. → **경험 주장 동사 활용형만 잡는 화이트리스트**(했·해보/해봤·썼·중이/중입)로 방향 반전. 막힘 유지: "6개월 사용해보았습니다"·"3년 사용했습니다"·"2년 썼는데"·"3년 사용 중입니다". 애매한 잔여는 막히는 쪽(fail-closed). 부수: `enrich_max_attempts` 2→3(주인 승인) — 상한 2면 밀도 외 게이트가 한 번만 걸려도 교정 기회가 사라진다.
- **FF5. 과밀 결정적 감산 백스톱 [확정 #48, 자가복원 4겹째]**: `enricher/density_fix.py` — 재시도를 다 써도 **density_high 하나만** 남았으면 LLM 재호출 없이(비용 0) 초과분을 대체어로 바꿔 밴드 안으로 되돌린다. 지시만으로는 LLM 출력이 확률적이라 수렴이 보장되지 않는다(#47이 지시만 고쳐 실패한 지점). 안전: 제목·H1 앞 공정위 고지·**모든 소제목**·첫 산문 등장 불가침 / 조사 형태가 갈리는데 뒤 글자가 애매하면 그 자리 건너뜀(`keyword_density` 조사 판정) / 감산 후 **5게이트 전부 재실행**해 미통과면 원문 복귀 / 대체어를 못 구하면 미적용하고 **사유를 로그에 남김**. 실측 재생: 4.33%→1.42%, 3.97%→1.43% 둘 다 5게이트 통과. ★대체어 산출(`keyword_substitute`)이 핵심 — 옛 `_short_form`은 단일 토큰에서 **자기 자신**을 반환해 "과하면 줄임말로 대체하라"가 no-op였다.
- **FF6. ★비용 낭비 방지 5종 [확정 #48, 주인 지시]**: 라이브 6회·15콜·3시간을 쓴 원인은 ①**확률적 출력을 n=1 라이브 실행으로 판단**하는 루프(2회차에 이미 결론에 필요한 표본이 있었는데 문구만 3번 더 바꿈) ②**LLM 사용량 미기록**(api_usage는 Imagen 전용·콘솔 로그는 마지막 시도 1회분 → 원화 환산 불가). → (1)**CLAUDE.md §7-3 발행 실패 대응 순서**: 서비스 복구 먼저(격리 키워드는 **그대로 두면** 다음 사이클이 다른 키워드 자동 선정 — `keyword-requeue`로 되살리지 말 것) → 오프라인 분석 → 라이브는 마지막 1회. (2)`cli experiment --draft N --samples M` — 저장 draft의 상품·SEO 재사용으로 **LLM만 변수 격리**, N샘플 분포를 표로. 기본 dry-run(비용 0)·DB 쓰기/발행/배포 없음. (3)migration **010** `api_usage.tokens_in/out` + `record_llm`/`llm_summary` — **재시도·형식오류·빈응답까지 매 호출 1행**(실패 호출도 과금). 단가는 설정 `llm_price_*_per_1m`이 있을 때만 환산(모르는 단가 날조 금지·§0). (4)본문 분량 예산 정합화 — 총 2,000~2,500자를 요구하며 상품 6~12개를 각 200~300자로 쓰라던 **명세 모순**이 초과(3,830~5,199자)의 진짜 원인. 상품 120~180자·총 2,600~3,200자로 수정 → 산문 25~30%↓. (5)캐시 관찰성.
- **FF7. 프롬프트 캐시는 제공자가 자동으로 건다 — 배선 복구 불요 [확정 #48, 실측 종결]**: `build_system_blocks`의 `cache_control`은 `_system_to_text`가 평문화하며 사라지지만(코드 확정), 실측 결과 **input 27,687 중 16,448(59%) 캐시 적중** — DeepSeek/OpenRouter가 자동 프리픽스 캐시를 적용 중. 조치 불요로 종결. ※응답의 `cached_tokens`를 안 읽어 **관측 자체가 불가능**했던 것이 진짜 갭이었다 → usage에 실어 로그·실험 요약에 표시.
- **FF8. 비표준 slug 근본수정 [확정 #48, 라이브 3건 적발]**: `slugify`가 ASCII로 못 옮기는 문자를 조용히 버리고 **남은 조각만으로** slug를 만들었고 해시 폴백은 '전부 유실'일 때만 걸렸다(부분 유실은 통과) → `/articles/1/`·`/tpu/`·`/1-2/`. 피해 3가지: 의미 전달 0 / **서로 다른 키워드가 같은 조각으로 충돌**('1…' 시작은 전부 `1`) / URL 품질. → **부분 유실도 폴백**(NFKD 분해 후 alnum 개수 비교·결합 악센트는 유실 아님). ★그것만으론 부족 — 시나리오 slug(=공개 URL 씨앗)는 **등록 시점에 굳은 `keyword_queue.slug`를 물려받는다**. `32인치모니터암`이 slug `32`로 **시나리오 없이 대기** 중이어서 다음 발행에 `/articles/32/`가 될 예정이었다 → `ensure_scenario_for_keyword`가 조각 slug를 자가교정(이 지점은 시나리오 없는 키워드만 도달 = 글·라이브 URL 부재라 안전). 운영자가 일부러 지정한 slug는 옛 함수 산출물과 정확히 일치할 때만 조각으로 판정해 보호. **이미 라이브인 3건은 주인 결정으로 URL 유지**(301 없이 바꾸면 색인·링크 단절, 301을 걸어도 크롤 예산 소모 — #46 DD1이 크롤 예산을 병목으로 확정).

## GG. 'validated=5게이트 통과' 구멍 봉인·씨앗 인치 변형·사람 추가 경로 경고 [확정] — 세션 #49 신규

> 08-04 발행 0편(미매핑 보류)을 파다가 **더 큰 결함**이 나왔다: 미매핑 키워드 글은 seo 게이트가
> skip된 채 validated가 되는데, 자동 승인은 게이트를 재검증하지 않아 **씨앗에 매핑을 추가하는
> 순간 미검증 글이 자동 발행**된다. 상세 EVENTS #49. [[autonomous-safe-system]]·[[critical-final-review]].

- **GG1. 08-04 발행 0편 원인 = 사람이 넣은 미매핑 키워드 [확정 #49, 로그·DB]**: `32인치모니터암`(큐 #34)은 07-27 **추천 다이얼로그로 사람이 추가**한 5건 중 하나(`추천(검색량 2760·씨앗 모니터암)`). `monitor-arm` 카테고리는 존재·공개인데 `seo_keywords.yml` secondary에 인치 변형이 없었고 `resolve_category`는 오매핑 방지를 위해 **정확 매칭**만 한다 → `unmapped` 보류. 침묵 정지가 아니라 fail-loud 정상 작동([ALERT] 발생).
- **GG2. ★무인 리필 vs 사람 추가의 비대칭이 사고 통로 [확정 #49, 코드·라이브 재현]**: `auto_pick_keyword`의 리필 분기는 미매핑·비공개 후보를 **자동 배제**한다(#45). 반면 사람 경로(대시보드 🎯추천/🆕추가·`keyword-add`·`--add-top`)는 운영자 판단 존중으로 제한하지 않는다 — 이 비대칭 자체는 설계 의도가 맞지만, **넣는 시점에 아무 신호가 없어** 미매핑 키워드가 조용히 큐에 쌓였다가 며칠 뒤 LLM 비용을 쓴 뒤에야 보류로 드러났다(07-27 추가 → 08-04 발행 0편). → 차단이 아니라 **넣는 순간 알린다**: `keyword_relevance.mapping_warning()` 단일 소스 + CLI 추가/추천 목록/`--add-top` + 대시보드 선택 창 **'발행' 칸**(고르기 전에 보임). 라이브 재현: 리필 적격 18 / 미매핑 20 — 리필은 20개를 자동으로 걸러내고 있었다.
- **GG3. ★★seo 게이트 skip은 '통과'가 아니다 — 자동 승인 봉인 [확정 #49, 운영 실데이터 반증]**: 미매핑 키워드는 `cli._keyword_seo_cfg`가 `{}`를 돌려 `validator.check_seo`가 **skip(=pass)**되고 그대로 `validated`가 된다. `auto_approve`는 게이트를 재검증하지 않고 status만 보므로, 나중에 그 키워드가 씨앗에 추가되면 **미검증 글이 재검증 없이 무인 발행**된다. 반증 실측: 가드를 끄면 draft #48이 `ok=True`(승인 가능)가 된다 — 그 글은 산문 대표키워드 **0회**·소제목 0개·도입부 0으로 게이트 적용 시 3 issue 실패. → 저장된 `validation_report`에서 seo skip이면 보류(`code='seo_unverified'`). **보고서 없음·손상도 동일 보류**(fail-closed — 이 모듈의 미탐<오탐 정책. 운영 43/43이 보고서를 보유해 회귀 없음). 해소는 **재생성**뿐(매핑만으론 안 풀린다).
- **GG4. 갭은 3회 발동했으나 실질 피해는 1건 [확정 #49, 사후 실측·1차 보고 정정]**: 이 상태였던 draft는 4(게이밍의자·published)·26(여름이불·published→#46이 이미 자동비공개)·48. **1차엔 "미검증 2편이 라이브"로 보고했으나 실측하니 과장**이었다 — 게이밍의자 10회·1.24%·소제목 6개, 여름이불 7회·1.34%로 **둘 다 seo 기준 충족**. 즉 지금까지는 **우연히** 통과해 온 것이고, 가드의 필요성은 draft 48이 실증한다.
- **GG5. 사람 1클릭 승인은 막지 않고 경고한다 [확정 #49, §2-마 준수]**: `cmd_approve`는 `eligible()`을 거치지 않아 자동 승인을 봉인해도 사람이 승인하면 미검증 글이 라이브로 간다(자체 재검수 적발). 화면엔 '5게이트 통과'로 보이는 게 문제의 핵심이라, **차단이 아니라 승인 직전 경고**로 해결 — 사람 판단이 최종 권한이라는 §2-마를 지키면서 정보 비대칭만 없앤다. 판정은 `auto_approve.seo_unverified()` 단일 소스.
- **GG6. 씨앗 인치 변형 등재 = 정확 매칭 설계의 필연적 유지비 [확정 #49, 네이버 실측]**: `monitor-arm` secondary에 32/27/24/34인치·고중량·폴타입 6개 추가(하한 300·20건 조회). 채택 기준은 기존 씨앗 정책 그대로 — 브랜드 5종(노스바유·어고트론·NB·알파스캔·한성) 제외(알리 공급 불가·헛트래픽), 정보성('모니터암설치') 제외, desk 양다리('모니터암책상') 제외(#45 교차 중복 금지). ★부분 매칭으로 바꾸는 게 근본 해결처럼 보이지만 **오매핑 위험이 더 크다**(설계 주석이 명시) — 정확 매칭을 유지하고 GG2 경고로 누락을 조기에 잡는 쪽을 택했다.
- **GG7. 리필 풀은 씨앗 secondary가 상한 [관찰 #49, 라이브 계측]**: 정확 매칭 때문에 무인 리필 후보는 **yml secondary에서만** 나온다(네이버가 준 40건 중 20건이 미매핑으로 탈락). 현재 적격 18개 = 하루 1편이면 **약 18일치**. 소진되면 생성 0편·[ALERT]. 씨앗 보강이 곧 발행 지속성이다.
- **GG8. #48 본문 예산 축소 부작용은 재현되지 않음 [관찰 #49, n=2]**: #48이 최우선 관찰로 남긴 `min_count=4` 여유 축소는 08-01(1인용밥솥 3,695자)·08-03(스텐도마 3,269자) **둘 다 시도 1/3 통과**로 재현되지 않았다. 본문도 #48 예산(2,600~3,200) 상단 이상 → **예산 상향 불필요**. 08-04 건은 seo skip이라 관찰 표본에서 제외(로그의 "게이트 통과"가 skip일 때도 찍히는 점은 판독 주의).

## HH. 무인 리필 캐시 강등 봉인·카테고리 균형·도달 불가 씨앗 가시화 [확정] — 세션 #50 신규

> GSC 실데이터로 "도마 6편 상위 노출 0"을 확인하고 편중 원인을 파다가 **원인이 2단**임이 드러났다:
> 무인 사이클이 `load_secrets`를 부르지 않아 리필이 **한 달간 검색량 없이** 씨앗 파일 순서대로
> 글감을 골라왔다. 상세 EVENTS #50. [[autonomous-safe-system]]·[[critical-final-review]].

- **HH1. ★★근본 원인 = `cmd_auto_cycle`의 `load_secrets` 누락 [확정 #50, 3중 증거]**: 무인 사이클이 secrets를 로드하지 않아 리필(`auto_pick_keyword`→`recommend`)이 네이버 자격증명 없이 조회 → 실패 → `recommend`가 **예외를 삼켜 캐시(yml secondary)로 자가강등**. 증거 ①코드 경로(`naver_searchad`가 "load_secrets 선행 가정"을 명시) ②데이터(07-01~27 리필 투입 10건이 **전부 `score=0`** = volume None = 캐시) ③순서(yml secondary 1~6번과 실제 발행 순서 완전 일치). **글 생성 경로는 자체적으로 load_secrets를 불러 LLM이 멀쩡히 돌았기 때문에 한 달간 은폐**됐다 — [[project_verify_script_load_secrets]]가 경고한 함정의 운영 코드판.
- **HH2. 캐시 강등 시 정렬이 무력화돼 카테고리를 통째로 소진 [확정 #50, 큐 이력 시각]**: 강등되면 winnable 점수가 전부 동률(-1.0)이라 **yml에 적힌 순서 그대로** 뽑힌다. 실측 — 리필 시각 `02:11 UTC(=11:11 KST)`로 사람 추가와 정확히 구분됨: 07-01~03 `desk` 3연속(게이밍책상·1인용컴퓨터책상·서재책상), **07-20~27 `cutting-board` 6연속**(나무도마·스텐도마·도마추천·엔드그레인도마·실리콘도마·TPU도마). `cutting-board` 씨앗이 #45에 추가되며 알파벳순 1위가 된 시점과 일치. GSC 실측으로 그 6편은 **상위 노출 0편**·씨앗 소진율 67%.
- **HH3. ★카테고리 균형은 '사용 횟수' 기준 — 1차 설계 자체 기각 [확정 #50, 실데이터 반증]**: 처음엔 "마지막 사용 시점이 오래된 카테고리 우선"으로 설계했으나 실데이터에 넣자 **도마를 또 뽑았다** — 누적 6편을 썼는데도 마지막 시점만 보면 우선 대상이 됐기 때문. → 키를 `(직전 카테고리면 1, 사용 횟수, -winnable 점수)`로 전환. 점수는 **동률 판정에만** 쓰고 후보 **집합은 불변**이라 적격이 하나뿐이어도 선택된다(§0 멈추지 않음). 리필 경로에만 적용하고 사람 경로는 §2-마 판단 존중으로 제외(대시보드 `auto_forecast`는 pending 큐만 보므로 화면·실제 불일치 없음).
- **HH4. 조용한 캐시 강등을 fail-loud로 [확정 #50]**: 원인(HH1)을 고쳐도 네이버가 실제로 다운되면 또 조용히 강등된다. `auto_pick_keyword`가 `degraded`로 보고 → digest `refill_degraded` + [ALERT] 로그 + 텔레그램. **발행 자체는 되므로 `abnormal`에는 넣지 않는다**(#49 과민 경보 교훈) — 반려 격리와 같은 '발행 여부 무관 독립 경보'.
- **HH5. GSC 실데이터 — 색인 4→18, 첫 검색 유입 [확정 #50, 주인 콘솔 실측]**: #46(07-20) 색인요청 11개가 **전환됨**(색인 4→**18**, +14 중 3개는 자연 색인). 3개월 누적 노출 73·클릭 **2**·평균 게재순위 **18.2**(구글 2페이지 초반). 클릭을 낸 글은 **「학생용의자 추천 가이드」**(CTR 16.7%) + 홈. 미색인 34 중 22는 정상(robots 차단 20=`/go/`·404 1·리디렉트 1), 실제 대기는 **12**(발견됨-미색인 11·크롤됨 1). ★병목이 "구글이 모른다"에서 **"알지만 상위에 안 띄운다"**로 이동. 노출된 검색어 8개가 **전부 씨앗에 없는 롱테일 변형**(tpu 도마 추천·자취방 책상·미니 밥솥 추천 등)이나 표본이 작아 가설 수준.
- **HH6. laptop-stand 카테고리 공개 [확정 #50, 주인 결정·라이브]**: draft 3개 실측 결과 **재고 부족이 draft 사유가 아니었다**(빨래건조대 45·노트북거치대 34·미니제습기 28개 — 공개된 6개(23~46)와 동등, 가이드 1,844~2,106자). 그 중 **씨앗 8개를 가진 laptop-stand만** 공개(나머지 둘은 씨앗이 없어 리필 기여 0). 결과: 공개 카테고리 6→7, sitemap 39→40, 라이브 200, 기존 글 영향 0. 리필 적격 18→**20**, 10일 시뮬레이션 카테고리 **4종→5종**. 나머지 2개는 노트북거치대 색인 결과를 보고 판단(저비용 실험).
- **HH7. 도달 불가 씨앗 = 무인 추천이 영원히 못 집는 secondary [확정 #50, 실측 75개 중 5개]**: `keyword_research`는 연관검색어를 core **부분문자열**로 거른다(`core_ns not in keyword_ns` → `no_core`). 따라서 어느 카테고리의 core도 포함하지 않는 secondary는 네이버가 아무리 추천해도 후보가 못 된다 — `monitor-stand`(core=받침대) '모니터 스탠드'·'모니터 라이저', `monitor-arm`(core=모니터암) '모니터 브라켓'·'모니터 마운트', `laptop-stand`(core=거치대) '노트북 스탠드'. **monitor-stand는 5개 중 2개(40%)가 죽어 실질 3개**뿐이고 2개는 이미 소진 → 오늘 적격 후보 0개였던 이유. → `lint_alignment`에 code `unreachable` 추가(정규화는 **운영 필터와 같은 함수** `keyword_research._ns`를 써서 드리프트 원천 차단). ★**삭제하지 않는다** — 그 secondary들은 **매핑 사전 역할이 유효**(사람이 추가하면 `resolve_category`가 찾아줌)하므로 고칠 오류가 아니라 알고 있어야 할 정보. 그래서 hard(FAIL) 아닌 경고.
- **HH8. 씨앗 보강은 재료 없음 — 자체 기각 [확정 #50, 전수 검토]**: '적게 쓴 카테고리(monitor-stand) 씨앗 보강'을 착수했으나 후보 3건(키보드받침대·컴퓨터받침대·컴퓨터본체받침대)이 **전부 다른 제품**이라 오매핑(글은 키보드받침대인데 모니터 받침대 상품이 붙음) → 기각. 미매핑 탈락 20건 전수 검토 결과 적합한 건 **전부 office-chair**(이미 9회로 과포화)이고, 걸러야 할 것이 섞여 있었다 — `허리디스크의자`(**비YMYL 원칙 위반 소지**)·`책상가격`/`의자브랜드`(정보성)·`쿠쿠미니밥솥`(브랜드)·`PC방의자`/`회의실의자`(1인가구 타겟 부적합). ★**TODO의 "보강 후보 = 미매핑 20건" 항목을 정정**해야 한다(그대로 넣으면 편중 악화).
- **HH9. LLM 단가 입력은 불요 [확정 #50, 주인 판단]**: 입력 UI 자체가 없어(대시보드 설정 23항목에 없음·config.json에 키 없음) "주인 입력 대기"로 이월돼 온 항목. 실측 사용량(08-01~05, 7콜·in 68,061·out 28,269 → 월 in 40만·out 17만 추정)으로 **월 수백 원 수준**임을 확인하고 주인이 불요 결정. 토큰 수는 단가 없이도 기록되므로 **이상 급증 감지는 유지**된다. 재검토 시점 = 발행량이 하루 여러 편이 되거나 더 비싼 모델로 전환할 때.
- **HH10. IndexNow 통지 생략은 오탐 [관찰 #50]**: 배포 중 "키 파일 미라이브"로 통지가 생략됐으나 키 파일은 실제 **라이브 200**. 폴링이 4회×30초(최대 90초)라 CI 배포 완료를 못 기다린 것으로 보이나 **첫 시도부터 실패한 이유는 [확인 불가]**. Google은 IndexNow를 지원하지 않으므로(F6) 색인 병목과 무관 — 우선순위 낮음.

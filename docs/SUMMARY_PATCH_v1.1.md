...
### L. 1인칭·사진 정책 재설계 — 5건 [확정 세션 #6]

| # | 결정 |
|---|------|
| L1 | 글 톤 — 위키바이형 정보 분석 기본 (1인칭은 본인 실보유 5~10개에만 선택적 액센트) |
| L2 | 사진 정책 — 페르소나 인테리어 사진 6~9장 사이트 전체만, 상품 사진은 쿠팡 공식 위젯 |
| L3 | validator/truth — 1인칭 검출 시 `owned_products` 메타 없으면 fail (기본 차단) |
| L4 | 벤치마크 차용 — Wirecutter·오늘의집은 형식·톤 영감만, 위키바이가 실질 베이스 |
| L5 | E8 축소·D5 완화 — E8 한국어 1인칭 전면 허용 폐기, D5 시나리오당 사진 의무 폐기 |

**폐기**: E8 (한국어 1인칭 전면 허용) · D5 (시나리오당 사진 1~3장 의무).

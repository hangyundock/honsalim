...
| 영역 | 값 | 최종 확인 세션 |
|------|----|---------------|
| 진행 단계 | **Phase 1 ~95% + Phase 2 핵심 모듈 12개·회귀 172 PASS** [확정] | #4 (2026-05-28) |
...
| Phase 2 핵심 모듈 12개 (세션 #3~#4) | cli · common/{config,logging,grading,db} · validator/{truth,schema,disclosure,links} · writer/{state_machine,article_writer} · collector/scenario_loader · enricher/{prompt_loader,claude_client,meta_extractor} · builder/jsonld | #4 |
| Phase 2 회귀 테스트 | **172/172 PASS** [확정] — validator 39 + state_machine 13 + scenario_loader 11 + enricher 13 + meta_extractor 31 + jsonld 23 + db 12 + cli 16 + article_writer 14. 9 test 파일 | #4 |
| doctor 보강 (세션 #4) | §9 prompt_templates 6종 + §10 Phase 2 모듈 진입점 17개 + §11 state_machine 매트릭스 + §12 tests 로드 가능 — Phase 2 진입 게이트 자동 점검 (BACKEND §9 [확정]) | #4 |
| builder.jsonld (세션 #4) | `build_article_jsonld(meta, scenario, ...) → JSON-LD 문자열` — POLICY §4·VALIDATOR §8 10필드 모두 충족·validator schema 게이트 정합 검증 통과 | #4 |

...
## 7. 운영 모델 — 세션 #2 갱신 / #38 무인 표준 순서 확정

### ★★★ 무인 가동 표준 작업 프로세스 [확정 #38 — 주인이 수십 번·매 세션 지시. 이게 작업의 default. Claude 절대 딴소리·우회·되묻기 금지]

> **★상세 표준 = `docs/AUTOMATION.md` — 무인/자동화 작업을 안내하기 전에 매 세션 반드시 먼저 읽을 것.** 아래는 요약.
> **키워드도 사람이 머리로 짜내지 않는다 — 시스템에 씨앗 자료 + 추천 엔진이 이미 있다.**
> (#37·#38 반복해서 '키워드를 직접 입력'·'글을 먼저 수동 생성' 같은 틀린 안내를 해 주인이 매 세션 같은 설명을 반복 → 강한 비판·신뢰 손상. 인수인계 단절이 핵심 문제.)

**1) 키워드 선정 = 자동(사람이 머리로 안 짠다).**
- 씨앗 자료: `src/collector/seo_keywords.yml` — 카테고리별 대표키워드(primary) + 네이버 연관검색어(secondary). 운영자는 가끔 다듬기만.
- 추천 엔진: `writer/keyword_recommender.recommend()` — 씨앗 → 네이버 실시간 검색량 → winnable 점수(검색량×경쟁가중, 경쟁 낮은 '틈' 우선) → 정렬.
- 무인 자동 선정: `auto_pick_keyword()` — 대기 큐(쿠팡 첨부 키워드 우선) → 없으면 추천에서 자동 보충. 큐가 비어도 안 멈춘다.
- 반자동: 🎯추천 키워드(대시보드) / `keyword-recommend --add-top`(CLI) = 네이버 추천 N건에서 택1해 큐에 추가.

**2) 완전 무인(auto_mode ON) 순서 — 운영자 거의 무관여:**
1. (선택) 씨앗 점검 — seo_keywords.yml 카테고리 대표 키워드 유지관리
2. (선택) 쿠팡 배너 미리 첨부 — 🛒쿠팡 첨부(저장)(키워드는 '대기' 유지·비용0)
3. 상단 무인 ON + 예약
4. → 매일 auto-cycle이 **키워드 자동 선정 → 상품 수집(쿠팡+알리) → 글 생성 → 자동 승인 → 발행**. 운영자 무관여.

**★금지(반복 실수 — 절대 하지 말 것):**
- 키워드를 '사람이 직접 머리로 입력하라'고 안내 금지(🆕키워드 추가는 보조 수단일 뿐, **추천 엔진이 기본**).
- '5편 채우려 글을 먼저 ✨수동 생성하자'류 우회 금지(글 생성은 스케줄러가 자동·`min_published`는 자동생성된 글의 '승인'만 보류).
- ★**한 키워드가 계속 반려된다고 그 키워드를 붙들고 라이브 재생성을 반복하지 말 것** (아래 §7-3).

**★★3) 발행 실패 시 대응 순서 [확정 #48 — 주인 지시. 순서를 바꾸지 말 것]**

주인의 목표는 **"오늘 발행이 나가는 것"**이지 특정 키워드를 살리는 게 아니다. 순서가 뒤집히면
비용·시간이 폭증한다(#48 실측: 병리 키워드 하나에 라이브 15콜·3시간 — 정상 키워드였으면 1회로 끝).

1. **먼저 서비스 복구** — 반려 상한(3회)에 도달한 키워드는 시스템이 이미 `failed`로 **자동 격리**해
   대기 큐에서 빼 준다. **그대로 두면** 다음 사이클이 다른 pending 키워드를 자동 선정해 발행이 재개된다.
   ★`keyword-requeue`로 되살리지 말 것 — 그게 같은 실패를 반복시킨 원인이다(#48: 3번 requeue).
   과거 발행 성공 유형(제품명 복합어: 나무도마·공부의자 등)이 안전하다.
2. **문제 키워드는 격리 상태로 두고 오프라인 분석** — `cli experiment --draft <id> --samples N`
   (기본 dry-run·비용 0, 실행은 `--no-dry-run`)으로 **분포를 먼저 측정**한다. 게이트·백스톱 변경은
   저장된 본문(drafts)으로 **비용 0** 재생 검증이 가능하다.
3. **라이브 재생성은 마지막**, 그것도 오프라인에서 가설이 확정된 뒤 1회 확인용으로만.
   확인이 끝나면 그때 `keyword-requeue`로 되살린다.

★**금지: 확률적 출력을 n=1 라이브 실행으로 판단하기.** 프롬프트를 고쳐 1회 돌려보고 실패하면
또 고쳐 1회 돌리는 루프는 #48에서 6회 연속 실패했다. **먼저 N샘플 분포를 보라**([[critical-final-review]]).

**유일한 결정 1개:** `auto_approve_min_published`(첫 N편 사람검수) 유지(5)/끄기(0=완전무관여). 위험은 정보로만, 결정은 주인. DECISIONS C21·C22·C23 · [[autopublish-operational-model]]·[[autonomous-safe-system]].
...
- **`docs/AUTOMATION.md` — ★무인 자동화 전체 작업 프로세스 표준 (완성 #38). 무인/자동화 작업 안내 전 매 세션 필독.**
...
- Python: 3.10 (32-bit, 시스템 Python, TIMA·AutoBlog 공유)
- **★"환경 변경 금지"의 실제 이유 [주인 설명 #52]**: 키움 HTS API(TIMA)가 **32-bit Python을 요구**해, 공유 환경을 건드리면 타 프로젝트가 깨질 것을 우려한 규칙이다. 따라서 ①**Python 자체(32-bit 3.10)는 유지** ②그 외(패키지 추가 등)는 **기존 프로그램 작동·결과에 문제없음을 검증하면 향상된 방식 허용**. 규칙은 이유를 알고 적용할 것 — 이유가 적용되지 않는 영역까지 과잉 준수하지 말 것. 단 공유 site-packages이므로 설치·업그레이드 전 기존 프로젝트(TIMA·AutoBlog·K-Content)의 의존 버전과 충돌 여부 확인 + 설치 후 검증이 조건.
- 가상환경 미사용
- 인코딩: 모든 .py / .md UTF-8
...
| 2026-08-07 | §12 "환경 변경 금지"의 이유 명시(키움 32-bit 공유 충돌 방지) + 검증 조건부 향상 허용 — 주인 지시 #52 | Claude Fable 5 |

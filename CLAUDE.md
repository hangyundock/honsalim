...
### ★★★ 무인 가동 표준 작업 순서 [확정 #38 — 주인이 수십 번·매 세션 지시. 이 순서가 작업의 default. Claude는 절대 딴소리·우회·되묻기 금지]

> **주인의 무인 운영 모델 = 아래 순서가 작업의 기본값이다.** 무인 가동 관련 작업을 할 때 Claude는 매 세션 이 순서대로 진행하고, **"글을 먼저 수동 생성하라" 등 다른 순서·새 프로세스를 제안하거나 운영 방식을 되묻지 말 것.** (이미 메모리 [[autopublish-operational-model]]·DECISIONS C21에 있는데도 반복해 어겨 주인이 매 세션 같은 설명을 반복함 — #38 강한 비판. 신뢰 손상이 핵심 문제.)

1. **키워드 등록** — [키워드] 탭 🆕키워드 추가(또는 🎯추천)로 대기 키워드 선정
2. **쿠팡 첨부(저장)** — 그 키워드 선택 → 🛒쿠팡 첨부(저장)로 쿠팡 배너 HTML 저장 (글 생성 안 함·비용0)
3. **적재 반복** — (키워드+쿠팡) 쌍을 원하는 만큼 쌓음
4. **무인 ON** — [설정] auto_mode ON + ⏰예약(11:00)
5. **무관여** — 스케줄러가 매일 그 키워드로 자동 글 생성·발행. 주인은 무관여

→ **글 생성(✨)은 스케줄러가 자동으로 한다. 사람이 미리 ✨글 생성을 누르지 않는다.**
→ 이미 코드 전부 구현됨(DECISIONS C21)·**코드 수정 불필요·설정/운영만**. 저장한 쿠팡은 자동 글에 항상 포함(끊김 없음) → 주인 모델은 빈 글이 안 생긴다.
→ **유일한 결정 1개**: `auto_approve_min_published`(첫 N편 사람검수 초기 안전장치) **유지(기본5) vs 끄기(0=완전무관여)**. 위험은 정보로만 제시, 결정은 주인(C13 수동운영을 뒤집는 주인 결정). [[autonomous-safe-system]]·[[assist-not-overstep]].
